	...
	if (myfile.is_open())
	{
		...
		while (! myfile.eof() )
		{
			...
			for (unsigned int i = 0; i < line.length(); i++)
			{
				if (line[i] == '.')
				{
					Dot* dot = new Dot(Vector2f(static_cast<float>(i * 22), static_cast<float>(lineIndex * 22)));
	...
	if (myfile.is_open())
	{
		...
		while (! myfile.eof() )
		{
			...
			for (unsigned int i = 0; i < line.length(); i++)
			{
				if (line[i] == 'o')
				{
					BigDot* dot = new BigDot(Vector2f(static_cast<float>(i) * 22, static_cast<float>(lineIndex) * 22));
